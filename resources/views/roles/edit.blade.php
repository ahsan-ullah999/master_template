...
<div class="content mt-4 px-4">
  ...
  <form method="post" action="{{ route('roles.update',$role->id) }}" class="col-md-6">
    @csrf
    <div class="mb-3">
        ...
        @if ($permissions ->isNotempty())
            @foreach($permissions as $permission)
                <div class="form-check justify-content-between ">
                    <input {{ ($haspermissions->contains($permission->name)) ? 'checked' : '' }} id="permission-{{ $permission->id }}" class="form-check-input " type="checkbox" name="permission[]" value="{{ $permission->name }}">
